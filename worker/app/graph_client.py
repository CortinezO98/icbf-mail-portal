...
class GraphClient:
    ...
    async def _request(self, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        # Reusa un solo cliente por request con retries
    ...
    async def list_attachments(
        self, mailbox_email: str, message_id: str
    ) -> list[dict[str, Any]]:
    ...
    async def get_attachment(
        self, mailbox_email: str, message_id: str, attachment_id: str
    ) -> dict[str, Any]:
    ...
    async def create_subscription(
        ...
        *,
        ...
        if resp.status_code not in (200, 201):
            logger.error(
                "create_subscription failed: %s %s", resp.status_code, resp.text
            )
            raise RuntimeError("Graph create_subscription failed")
        return resp.json()

    async def renew_subscription(
        self, subscription_id: str, expiration_datetime_iso: str
    ) -> dict[str, Any]:
        ...
        if resp.status_code != 200:
            logger.error(
                "renew_subscription failed: %s %s", resp.status_code, resp.text
            )
    ...
    async def get_subscription(self, subscription_id: str) -> dict[str, Any]:
        ...
        if resp.status_code != 200:
            logger.error(
                "get_subscription failed: %s %s", resp.status_code, resp.text
            )
...
class GraphClient(GraphClient):
    def _folder_ref(
        self, *, folder_code: str, graph_folder_id: str | None
    ) -> str:
        # Preferimos graph_folder_id si existe; si no, usamos well-known name.
        if graph_folder_id:
    ...
    async def messages_delta_page(
        ...
        *,
        ...
        if url:
            ...
            folder_ref = self._folder_ref(
                folder_code=folder_code,
                graph_folder_id=graph_folder_id,
            )
            delta_url = (
                f"{GRAPH_BASE}/users/{mailbox_email}"
                f"/mailFolders('{folder_ref}')/messages/delta"
            )
            params = {
                "$top": str(int(page_size)),
                "$select": "id,conversationId,receivedDateTime,subject,from,hasAttachments",
            }
            headers = await self._headers()
            headers["Prefer"] = f"odata.maxpagesize={int(page_size)}"
            resp = await self._request(
                "GET", delta_url, params=params, headers=headers
            )
