...
$status = isset($status) && $status !== '' ? strtoupper(trim((string)$status)) : null;
...
if (!function_exists('isEscalatedFilter')) {
    function isEscalatedFilter(?string $status): bool {
        $status = strtoupper(trim((string)$status));
        return in_array($status, ['ESCALADO', 'ESCALATED'], true);
    }
}

if (!function_exists('currentCasesQueryParams')) {
    function currentCasesQueryParams(): array {
        return is_array($_GET ?? null) ? $_GET : [];
    }
}

if (!function_exists('buildCasesUrl')) {
    function buildCasesUrl(array $overrides = []): string {
        $params = currentCasesQueryParams();
                ...
                <div>
                    ...
                    <div class="text-muted d-flex align-items-center gap-2 flex-wrap">
                        <span>
                            ...
                            <?php if ($status): ?>
                                Filtro: <?= esc($status === 'ESCALATED' ? 'ESCALADO' : $status) ?>
                            <?php else: ?>
                                Todos los estados
                            <?php endif; ?>
                        </span>
            ...
            <div class="d-flex justify-content-end gap-2 flex-wrap">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li>
                            <a class="dropdown-item <?= $status === null ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl(null)) ?>">
                                <i class="bi bi-circle-fill text-secondary me-2"></i>Todos
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'NUEVO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('NUEVO')) ?>">
                                <i class="bi bi-circle-fill text-primary me-2"></i>Nuevos
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'ASIGNADO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('ASIGNADO')) ?>">
                                <i class="bi bi-circle-fill text-warning me-2"></i>Asignados
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'EN_PROCESO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('EN_PROCESO')) ?>">
                                <i class="bi bi-circle-fill text-info me-2"></i>En Proceso
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'ESPERANDO_INFO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('ESPERANDO_INFO')) ?>">
                                <i class="bi bi-circle-fill text-warning me-2"></i>Esperando información
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= isEscalatedFilter($status) ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('ESCALADO')) ?>">
                                <i class="bi bi-circle-fill text-danger me-2"></i>Escalado
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'RESPONDIDO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('RESPONDIDO')) ?>">
                                <i class="bi bi-circle-fill text-success me-2"></i>Respondidos
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item <?= $status === 'CERRADO' ? 'active' : '' ?>"
                               href="<?= esc(buildStatusUrl('CERRADO')) ?>">
            ...
            <div class="row g-2 align-items-center">
                ...
                <div class="col-lg-8">
                    <form method="GET" action="<?= esc(url('/cases')) ?>" class="row g-2 justify-content-end" id="searchForm">
                        ...
                        <div class="col-md-auto">
                            <button class="btn btn-sm btn-outline-secondary w-100" type="button" id="refreshBtn">
        ...
        <div class="card-body p-0">
            <?php if (empty($cases)): ?>
                <div class="text-center py-5">
                    ...
                    <a href="<?= esc(url('/cases')) ?>" class="btn btn-brand">
                    ...
                    <table class="table table-hover align-middle mb-0" id="casesTable">
                        <thead>
                            <tr class="table-light">
                                <th class="ps-4" style="width: 180px;"># Caso</th>
                                <th>Asunto</th>
                        ...
                        <tbody>
                            <?php foreach ($cases as $c): ?>
                                ...
                                $statusCode = strtoupper(trim((string)($c['status_code'] ?? '')));
                                ...
                                ?>
                                <tr class="case-row <?= $priorityClass ?>" data-id="<?= $caseId ?>">
                                    <td class="ps-4">
                                        ...
                                        <div class="text-nowrap">
                                            ...
                                            <?php if (!empty($receivedAtRaw)): ?>
                                                <div class="text-muted small">
                                                    <?= date('H:i', strtotime((string)$receivedAtRaw)) ?>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="text-nowrap">
                                                ...
                                                <?php if (!empty($lastActivityRaw)): ?>
                                                    <div class="text-muted small">
                                                        <?= date('H:i', strtotime((string)$lastActivityRaw)) ?>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                            <a href="<?= esc(url('/cases/' . $caseId)) ?>"
